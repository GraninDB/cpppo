...
"""
enip.omron	-- Implements a Omron-like PLC subset
...
from . import ucmm
from .omron_parser import ( UDINT, DINT, UINT, INT, USINT, SINT, REAL, EPATH, STRING, SSTRING, typed_data,
...
class Omron( Message_Router ):
    """A Omron Controller implementation of the CIP Message Router (Class 0x02, Instance 1).  This
    ...
    Omron-specific commands (eg. Read/Write Tag [Fragmented]), it should be (or it won't be able to
    ...
    def reply_elements( self, attribute, data, context ):
        ...
        *Omron internally determines the correct number of elements to return by subtracting the
    ...
    def request( self, data, addr=None ):
        ...
        if ( data.get( 'service' ) == self.RD_TAG_REQ
            ...
            return super( Omron, self ).request( data, addr=addr )
    ...
    def produce( cls, data ):
        """Expects to find .service and/or .<omron-command>, and produces the request/reply encoded to
        ...
        if ( data.get( 'service' ) == cls.RD_TAG_REQ
            ...
            result		= super( Omron, cls ).produce( data )
        return result
...
def setup( **kwds ):
    ...
    otherwise, use the default Identity (of a *Omron PLC).
